"""
Назначение: модуль "test rep parity" реализует продуктовый контур в зоне Discord/Telegram/общая логика (тесты).
...
def test_rep_service_keeps_same_escalation_payload_on_both_platforms() -> None:
    ...
    assert "Шаг по текущему нарушению" in discord_ui["preview_text"]
    assert "Общие предупреждения" in discord_ui["preview_text"]
    assert "Общие предупреждения" in discord_ui["moderator_result_text"]
...
def test_rep_renderers_include_preview_and_result_explanations() -> None:
    ui_payload = {
        "preview_text": "👤 Нарушитель: Target\n📘 Нарушение: Спам\n⚠️ Шаг по текущему нарушению: 1/5\n⚠️ Общие предупреждения: 7/5 до бана\n🧮 Будет применено сейчас: мут 6 ч. + предупреждение + штраф 10 баллов\n📈 Шаг по текущему нарушению после применения: 2/5\n📈 Общие предупреждения после применения: 8/5 до бана\n⏭️ Следующий шаг: При следующем таком нарушении наказание усилится: бан.",
    ...
    assert "Подробности по кейсу" in result_text
    assert "профиле пользователя" in result_text
    assert "Мут закончится" in violator_text
    assert "Как избежать усиления" in violator_text
