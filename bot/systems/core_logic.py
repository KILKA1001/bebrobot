...
def get_help_embed(category: str, visibility: HelpVisibilityContext | None = None) -> discord.Embed:
    ...
    if category == "points":
        ...
        lines = [
            ...
            "`/myfines` — переходный экран оплаты legacy-штрафов (если штраф не списан автоматически в кейсе).",
        ...
        description = [
            "Рейтинг должников выведен из основного продукта: вместо него используйте `/modstatus` для чтения активного статуса и `/rep` для создания новых кейсов.",
            "`/modstatus` — показать активные наказания, предупреждения, последние кейсы и legacy-штрафы; чужой профиль в сервере открывается только через reply, а в личке — только модератору по явному lookup.",
        ]
        if _help_can_use_rep(visibility):
