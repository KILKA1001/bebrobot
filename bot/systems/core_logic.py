...
def get_help_embed(category: str, visibility: HelpVisibilityContext | None = None) -> discord.Embed:
    ...
    if category == "points":
        ...
        lines = [
            "Рейтинг должников больше не используется. Теперь модерация ведётся через кейсы — основной вход это `/rep`.",
            ...
            "Активные наказания, предупреждения, историю нарушений и штрафы в банк по кейсам смотрите через `/rep` и историю пользователя.",
        ]
        if _help_can_create_fines(visibility) or _help_can_use_rep(visibility):
            ...
            if _help_can_use_rep(visibility):
                extra_lines.append("`/rep` — основная точка входа в модерацию: выбрать нарушителя через reply/mention, выбрать нарушение кнопками, увидеть preview, активное наказание и итоговый авторасчёт по типу нарушения из БД и числу предупреждений; наказание вручную здесь не вводится.")
        ...
        description = [
            "Рейтинг должников выведен из основного продукта: вместо него используйте `/rep`, кейсы модерации, активные наказания и историю пользователя.",
        ]
        if _help_can_use_rep(visibility):
            description.append("`/rep` — основная точка входа: reply/mention для цели, кнопки нарушения, preview до применения, авторасчёт наказания по типу нарушения из БД и числу предупреждений, активное наказание, следующий шаг эскалации и итоговый кейс без ручного выбора наказания.")
        if _help_can_create_fines(visibility):
