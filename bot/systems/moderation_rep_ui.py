...
REP_HOW_IT_WORKS_LINES: tuple[str, ...] = (
    ...
    "• Бот берёт правило из таблицы moderation_penalty_rules и показывает расчёт до применения.",
    "• Следующий шаг эскалации бот показывает сразу в предпросмотре.",
    "• Пока кейс не подтверждён, ничего не применяется.",
    "• Если расчёт выглядит неверно — отмените сценарий и проверьте правило в БД.",
...
def render_rep_start_text(*, target_selection_hint: str, compact: bool = False) -> str:
    if compact:
        return (
            "🛡️ /rep — интерактивная модерация\n\n"
            "1. Выбери нарушителя.\n"
            "2. Нажми тип нарушения.\n"
            "3. Проверь preview.\n"
            "4. Подтверди кейс.\n\n"
            "Наказание вводить вручную не нужно: бот берёт тип нарушения из БД, считает предупреждения и показывает итог до применения.\n\n"
            f"ℹ️ Цель: {target_selection_hint}"
        ).strip()
    return (
        ...
        "• Основа расчёта — выбранный тип нарушения из БД + текущее число предупреждений.\n"
...
def render_rep_violation_prompt_text(*, target_label: str, compact: bool = False) -> str:
    if compact:
        return (
            ...
            f"Цель: {target_label}\n"
            "Дальше бот сразу покажет preview: что было по предупреждениям, что станет и что будет при повторе."
        ).strip()
    return (
        "📘 Шаг 2: выберите вид нарушения кнопками.\n\n"
        f"Цель: {target_label}\n"
        "После выбора бот сразу покажет авторасчёт наказания, текущий счётчик предупреждений, "
        "следующий шаг эскалации и напомнит, что бот берёт правило из таблицы, а не из ручного ввода."
    ).strip()
...
def render_rep_preview_failed_text() -> str:
    return (
        ...
        "Попробуйте открыть сценарий заново; если расчёт повторно не появляется — проверьте правило в БД."
    ).strip()
