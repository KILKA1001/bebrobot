...
def _log_rep(level: str, *, message: str, provider: str, chat_id: int | None, actor_account_id: str | None, target_account_id: str | None, violation_code: str | None, selected_actions: list[str] | None, case_id: Any | None, error_code: str | None) -> None:
    log_method = getattr(logger, level)
    log_method(
        "%s provider=%s chat_id=%s actor_account_id=%s target_account_id=%s violation_code=%s selected_actions=%s case_id=%s error_code=%s",
        message,
        provider,
        chat_id,
        actor_account_id,
...
async def rep_command(message: Message) -> None:
    ...
    if not AuthorityService.has_command_permission("telegram", str(message.from_user.id), "moderation_mute"):
        await message.answer("❌ Команда /rep доступна только ролям модерации. Если доступ должен быть, проверь authority и попробуй ещё раз.")
        return
    _PENDING_REP[message.from_user.id] = PendingRepState(step="await_target", created_at=time.time(), payload={"chat_id": message.chat.id})
    _log_rep(
        "info",
        message="rep start",
        provider="telegram",
    ...
    await message.answer(
        ...
        "В личке отправьте @username / username / id. В группе удобнее ответить на сообщение нарушителя и отправить /rep.\n\n"
        "Дальше бот сам проведёт по шагам: нарушение → предпросмотр → подтверждение → итог.",
...
async def rep_callback(callback: CallbackQuery) -> None:
    ...
    if action == "cancel":
        ...
        await callback.message.edit_text("/rep отменён. Запусти команду заново, если нужно начать сначала.", reply_markup=None)
    ...
    if action == "violation":
        ...
        if not preview.get("ok"):
            _log_rep(
                ...
                target_account_id=((preview.get("target") or {}).get("account_id") if isinstance(preview.get("target"), dict) else None),
                violation_code=code,
                selected_actions=list(preview.get("selected_actions") or []),
                case_id=None,
                error_code=str(preview.get("error_code") or "preview_failed"),
            )
            await callback.answer(preview.get("message") or "Не удалось построить предпросмотр.", show_alert=True)
            return
    ...
    if action == "confirm":
        ...
        try:
            ...
            if not result.get("ok"):
                ...
                await callback.answer(result.get("message") or "Не удалось применить модерацию.", show_alert=True)
                return
            ...
            logger.exception(
                ...
                violation_code,
                list((((preview.get("ui_payload") or {}).get("selected_actions")) or [])),
                None,
                "apply_exception",
            )
            await callback.answer("Не удалось применить модерацию. Обнови экран, проверь логи и попробуй ещё раз.", show_alert=True)
            return
...
async def rep_pending_handler(message: Message) -> None:
    ...
    if not resolved or resolved.get("error"):
        await message.answer((resolved or {}).get("message") or "❌ Не удалось определить нарушителя. Попробуй ещё раз, обнови экран и проверь логи.")
        return
    ...
    await message.answer(
        "Шаг 2: выберите вид нарушения.\n\n"
        f"Цель: {resolved.get('label') or 'неизвестный пользователь'}\n"
        "Бот сам рассчитает предупреждения, текущее наказание и следующий шаг эскалации.",
