"""
Назначение: модуль "fines" реализует продуктовый контур в зоне Discord.
Ответственность: единая точка для сценариев и правил модуля без дублирования логики между платформами.
Где используется: Discord.
Пользовательский вход: команда /fines и связанный пользовательский сценарий.
"""
        ...
        if fine:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    "откройте `/modstatus` и нажмите кнопку «Оплатить штраф»"
                ),
...
async def finehistory(
    ...
    embed = discord.Embed(
        ...
        description=(
            "Здесь хранится история денежных штрафов. "
            "Для проверки текущего статуса и оплаты используйте `/modstatus`."
        ),
    ...
    embed.set_footer(text=f"История штрафов • Страница {page}/{total_pages}")
    await send_temp(ctx, embed=embed)
...
async def topfines(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• `/modstatus` — посмотреть активные наказания и открыть кнопку «Оплатить штраф».\n"
            "• `/finehistory [@пользователь] [страница]` — открыть историю штрафов.\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• Активные штрафы оплачиваются через единый экран `/modstatus`.\n"
            "• История кейсов и нарушений сейчас смотрится через `/rep` и журнал модерации; позже она будет вынесена в отдельный экран."
        ),
        inline=False,
    )
    embed.set_footer(text="Используйте `/rep`. Вызов записан в лог, чтобы убрать оставшиеся ссылки на /topfines.")
    await send_temp(ctx, embed=embed)
