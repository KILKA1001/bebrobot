...
-- Safe rollout for heterogeneous schemas: create missing account_id columns, then backfill.
...
-- 2) Ensure account_id column exists in target hot tables.
...
-- 3) Hot-path indexes by account_id (create only when table+column exist).
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='scores' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_scores_account_id ON scores(account_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='actions' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_actions_account_id ON actions(account_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='ticket_actions' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_ticket_actions_account_id ON ticket_actions(account_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bank_history' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_bank_history_account_id ON bank_history(account_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fines' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_fines_account_id ON fines(account_id)';
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='fine_payments' AND column_name='account_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS ix_fine_payments_account_id ON fine_payments(account_id)';
  END IF;
END $$;

-- 4) Backfill account_id from account_identities for discord-linked rows.
...
-- 5) Readiness checks (verify all counters are zero before NOT NULL):
...
-- 6) Strict mode (run only after checks == 0):
