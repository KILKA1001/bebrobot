...
| Roles admin | `/rolesadmin ...` | `/roles_admin ...` + текстовый alias `/rolesadmin ...` | partial | Базовый CRUD категорий/ролей и просмотр/выдача/снятие ролей доступны на обеих платформах. Явно отмечено различие в имени команды: Discord использует `/rolesadmin`, Telegram — `/roles_admin`, но для паритета Telegram принимает и `/rolesadmin`. Обновление каталога Discord-ролей выполняется внутри кнопочного сценария панели, без отдельной подкоманды. |
