...
| Council proposals | `/proposal` | `/proposal` | full | Единый сценарий внутри одной команды: разделы «Подать предложение», «Статус», «Архив решений», «Помощь». Подача идёт через форму и обязательный экран подтверждения перед отправкой; переходы выполняются только кнопками в рамках одного сценария. |
