...
| Moderation | `/rep` | `/rep` | partial | Единый moderation-service и одинаковый смысл шагов уже есть на обеих платформах: выбрать нарушителя → выбрать нарушение → увидеть авторасчёт наказания → подтвердить → получить итог и объяснение. Discord использует `discord.ui.View` + select, Telegram — pending-state + inline-кнопки. Временное ограничение Telegram зафиксировано явно: выбор нарушителя там начинается с reply/username/id ввода, потому что у платформы нет нативного user-select, но сам смысл шага и тексты preview/result остаются общими. |
