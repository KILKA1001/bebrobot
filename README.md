...
- Если OpenRouter временно недоступен/не настроен, Гуй теперь отвечает диагностической fallback-репликой в чате (и пишет подробную ошибку в логи), чтобы было понятно, почему нет полноценного AI-ответа.
...
- Для OpenRouter добавьте переменные окружения:
  - `OPENROUTER_API_KEY` — API-ключ OpenRouter (основной, обязателен для AI-ответов).
  - `OPENROUTER_MODEL` — опциональная модель (по умолчанию `qwen/qwen3-coder:free`).
  - `OPENROUTER_MODELS` — опциональная fallback-цепочка моделей через запятую.
