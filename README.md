...
- Если Gemini временно недоступен/не настроен, Гуй теперь отвечает диагностической fallback-репликой в чате (и пишет подробную ошибку в логи), чтобы было понятно, почему нет полноценного AI-ответа.
...
- Для Gemini добавьте переменные окружения:
  ...
  - `GEMINI_USE_FREE_TIER` — по умолчанию `1`: включает безопасную цепочку free-tier моделей (`gemini-1.5-flash`, `gemini-1.5-flash-8b`).
  - `GEMINI_MODEL` — опциональная приоритетная модель Gemini (если нужна своя, например `gemini-1.5-flash`).
  - `GEMINI_MODELS` — опциональный список fallback-моделей через запятую (добавляются перед дефолтами).
