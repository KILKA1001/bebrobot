...
- Для Groq добавьте переменные окружения:
  ...
  - `GROQ_MODEL` — опциональная модель для жёсткого pin на один model id (если задана, отключает дефолтную цепочку).
  - `GROQ_MODELS` — опциональная fallback-цепочка моделей через запятую (полностью переопределяет встроенный порядок; рекомендуется указать 2+ модели, чтобы переживать временные лимиты провайдера одной модели).
...
- Базовая модель для обычного текстового чата без медиа — `moonshotai/kimi-k2-instruct-0905`.
- Дефолтная текстовая fallback-цепочка Groq: `moonshotai/kimi-k2-instruct-0905` → `qwen/qwen3-32b` → `llama-3.3-70b-versatile`.
- Для сообщений с медиа анализ изображения и приоритетная media-цепочка завязаны на `llama-3.3-70b-versatile`: она используется как vision-модель по умолчанию, а media-backed fallback-порядок идёт как `llama-3.3-70b-versatile` → `moonshotai/kimi-k2-instruct-0905` → `qwen/qwen3-32b`.
