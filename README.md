...
- Для Groq добавьте переменные окружения:
  ...
  - `GROQ_MODEL` — опциональная модель (по умолчанию `moonshotai/kimi-k2-instruct-0905`).
  - `GROQ_MODELS` — опциональная fallback-цепочка моделей через запятую (рекомендуется указать 2+ модели, чтобы переживать временные лимиты провайдера одной модели).
