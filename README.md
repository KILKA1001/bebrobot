...
- Для Gemini добавьте переменные окружения:
  ...
  - Модель зафиксирована на `gemini-2.5-flash` (single-model режим).
  - `GEMINI_MODEL` и `GEMINI_MODELS` игнорируются и логируются как ошибка конфигурации, если отличаются от `gemini-2.5-flash`.
  - `GEMINI_USE_FREE_TIER` оставлен для обратной совместимости и не меняет итоговую модель.
